...
public class viewSalesInfo extends JFrame{
	JTable table;	
	DB conn = new DB();
	
	
	viewSalesInfo(String str){
...
System.out.println(" View Sales Information");
 try {
    ...
    cloname[i]=rsmd.getColumnName(i+1);
     ...
     String sale_id,quantity,sales_date,sales_price;
    // System.out.println("froloop ended");
     while(rs.next()) {
    	 sale_id=rs.getString(1);
    	 quantity =rs.getString(2);
    	 sales_date=rs.getString(3);
    	 sales_price=rs.getString(4);
    	 
   	  String row[]= {sale_id,quantity,sales_date,sales_price};
   	 model.addRow(row);
