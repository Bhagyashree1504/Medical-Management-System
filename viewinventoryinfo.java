...
public class viewinventoryinfo extends JFrame{
	JTable table;	
	DB conn = new DB();
	
	
	viewinventoryinfo(String str){
	// TODO Auto-generated constructor stub
	
        table = new JTable();
System.out.println(" View Inventory Information");
 try {
    ...
    cloname[i]=rsmd.getColumnName(i+1);
     ...
     String medi_id,quant,purchase_price,sell_price;
    // System.out.println("froloop ended");
     while(rs.next()) {
    	 medi_id=rs.getString(1);
    	 quant =rs.getString(2);
    	 purchase_price=rs.getString(3);
    	 sell_price=rs.getString(4);
    	 
   	  String row[]= {medi_id,quant,purchase_price,sell_price};
   	 model.addRow(row);
